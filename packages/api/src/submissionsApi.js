...
export const getMySubmissionsPage = async ({
  size = FEEDBACK_PAGE_SIZE,
  page = 0,
} = {}) => {
  const params = new URLSearchParams({ size: String(size), page: String(page) });
  const response = await apiFetch(`/submissions/mine?${params}`, {
    headers: authHeaders(),
  });
...
export const getAdminSubmissionCounts = async () => {
  const response = await apiFetch("/admin/submissions/counts", {
    headers: authHeaders(),
  });
  return handleResponse(response);
};

export const getAdminSubmissionsPage = async ({
  status,
  page = 0,
  size = FEEDBACK_PAGE_SIZE,
} = {}) => {
  ...
  const response = await apiFetch(`/admin/submissions?${params}`, {
    headers: authHeaders(),
  });
  return handleResponse(response);
};
...
export const approveSubmission = async (id, body) => {
  const response = await apiFetch(`/admin/submissions/${id}/approve`, {
    method: "POST",
    headers: authHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify(body),
  });
...
export const rejectSubmission = async (id, reason) => {
  const response = await apiFetch(`/admin/submissions/${id}/reject`, {
    method: "POST",
    headers: authHeaders({ "Content-Type": "application/json" }),
